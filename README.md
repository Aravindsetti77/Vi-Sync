...
- **AI-Powered Recommendations**: Uses the `all-MiniLM-L6-v2` Sentence Transformer model to deeply understand the context and themes of movie titles rather than relying on simple genres or tags.
...
3. **Install dependencies:**
   ...
   ```
   *Note: On first run, the SentenceTransformer model (`all-MiniLM-L6-v2`) will be downloaded locally. This might take a moment depending on your internet connection.*
...
2. **Text Embedding**: Recent movie titles are passed through the `sentence-transformers` model to generate mathematical vector embeddings.
